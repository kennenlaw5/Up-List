function summary(type, date, range) {
  ...
  for (var i = 0; i < range.length; i++) {
    ...
    if (range[i][0].split(' ')[0] !== '') {
      if (arr.length == 0) {
        ...
        if (!found) arr[arr.length] = range[i];
      }
    }
  }
  ...
  for (i = 0; i < arr.length; i++) {
    if (arr[i][1] === type) final[final.length] = arr[i][0];
    
    if (type === 'Not Assigned' && arr[i][1] === '') final[final.length] = arr[i][0];
  }
...
function sweep() {
  ...
  var name, lowerd, todayd, today, sheet, str, strd, strm, todaym;
...
function addCA() {
  ...
  range = sheet.getRange(3, 2, sheet.getLastRow()).getValues();
  ...
  name = ui.prompt('Add CA','Enter CA\'s name:', ui.ButtonSet.OK_CANCEL);
...
function fresh(){
  ...
  for (var i = 0; i < range.length; i++) {
    if (range[i] === 'Bull-Pen') { var bull = parseInt(i, 10) + 3; bullc = true; }
    else if (bullc === true) {
      if (range[i] === 'Available') { var avail = parseInt(i, 10) + 3; availc = true; break; }
    } 
    else if (range[i] === '') { break; }
  }
...
function skip(){
  ...
  for (var i = 0; i < range.length; i++) {
    if (range[i] === 'Bull-Pen') { var bull = parseInt(i, 10) + 3; bullc = true; }
      ...
      if (range[i] === 'Available') { var avail = parseInt(i, 10) + 3; i += 999; availc = true; }
    }
